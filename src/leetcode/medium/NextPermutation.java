...
public class NextPermutation {
    ...
    @Test
    public void debugNextPermutation() {
        assertTrue(Arrays.equals(nextPermutation(new int[]{1, 5, 1}), new int[]{5, 1, 1}));
    ...
    public int[] nextPermutation(int[] nums) {
        ...
        if (j == 0) {
            ...
            // 取从j...length - 1中找到比nums[j-1]大的最小值位置记为s；交换s与j-1位置元素；对j...length-1位置的元素重新排序

            // 由于j...length-1已经按照降序排列，从右往左找到第一个比num[j-1]大的元素即可
            int s = length - 1;
            while (nums[s] <= nums[j - 1]) {
                s--;
            }
            //
            // 交换
